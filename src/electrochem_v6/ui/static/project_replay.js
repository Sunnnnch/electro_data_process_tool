(function () {
  ...
  function parameterFields(ctx, params, dataTypes = []) {
    ...
    const byType = { LSV: ["lsv_target_current", "tafel_enabled", "tafel_range", "ir_compensation_enabled", "ir_manual_ohm"], CV: ["cv_scan_rate_v_s", "cv_cycle_numbers"], EIS: ["eis_circuit_model", "eis_randles_fit"], ECSA: ["ecsa_ev", "ecsa_last_n", "ecsa_avg_last_n", "ecsa_cs_value", "ecsa_cs_unit"], COUPLED: ["coupled_input_mode"] };
    ...
    const fields = Object.entries(params || {}).flatMap(([key, value]) => {
      ...
      const type = nullable ? definition.value_type : typeof value;
      ...
      let input;
      if (type === "boolean" && nullable) input = `<select ${attributes}><option value="" selected>—</option><option value="true">${escape(ctx, text(ctx, "preflight_status_yes"))}</option><option value="false">${escape(ctx, text(ctx, "preflight_status_no"))}</option></select>`;
      else if (type === "boolean") input = `<input ${attributes} type="checkbox" ${value ? "checked" : ""}>`;
      else if (options.length) input = `<select ${attributes}>${nullable ? '<option value="" selected>—</option>' : ""}${options.map((option) => `<option value="${escape(ctx, option)}" ${!nullable && String(option) === String(value) ? "selected" : ""}>${escape(ctx, option)}</option>`).join("")}</select>`;
