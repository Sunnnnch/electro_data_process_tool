...
当前版本：**7.0.1** · [本版发布说明与升级注意](docs/release_7.0.1.md) · [更新记录](CHANGELOG.md)
...
- 支持 `EIS` 的 `Nyquist` / `Bode` 绘图，以及 `Rs + (Rct || Cdl)` / `Rs + (Rct || CPE)` 两种单时间常数等效电路拟合和残差诊断
...
- 统一外观设置提供四种主题与跟随系统、独立字号和舒适/紧凑密度、可选背景网格；助手长文本和图表预览适配主题，科学图表导出保留白底。详见[外观说明](docs/appearance.md)。
...
1. 在“专业模式”点击“选择数据”，选 TXT/CSV 文件或文件夹，核对清单中的类型与勾选项。
...
- `Nyquist` 图
- `Bode` 图（幅值+相位）
- **等效电路拟合**：理想 `Rs + (Rct || Cdl)` 或非理想 `Rs + (Rct || CPE[Q,n])`；保存 R²、RMSE、接受阈值与模型限制
- 历史记录与结果输出
...
- 扩展多时间常数、Warburg 和感抗等更复杂 EIS 模型
- 为 README 补使用截图或流程图
- 实测 `PyInstaller` 打包链路
- `CV` 多圈自动分段与循环伏安参数提取
- 前端结果页展示跳过错误文件的详细列表
