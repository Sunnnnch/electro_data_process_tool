"""Synthetic end-to-end regression for the complete processing workflow.

Instrument text compatibility is covered separately by static fixtures in
``test_v6_instrument_text_fixtures.py``.  This test intentionally generates
deterministic curves so numerical and artifact assertions stay reproducible.
"""
...
def test_v6_e2e_synthetic_pipeline_all_types(tmp_path):
    ...
    payload = {
        ...
        "params": {
            ...
            "lsv_target_current": "10,100",
        },
    }
    preflight_response = process_service.preflight_process_folder(payload)
    assert preflight_response.get("status") == "success"
    preflight = preflight_response.get("preflight") or {}
    assert preflight.get("runnable") is True
    assert preflight.get("matched_counts") == {
        "LSV": 2,
        "CV": 1,
        "EIS": 1,
        "ECSA": 3,
    }
    assert len(preflight.get("by_type", {}).get("ECSA", {}).get("files") or []) == 3
    ...
    manifest_path = Path(result.get("manifest", {}).get("outputs", {}).get("run_manifest_path") or "")
    assert manifest_path.exists()
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert manifest.get("run", {}).get("data_types") == ["LSV", "CV", "EIS", "ECSA"]
    assert manifest.get("inputs", {}).get("matched_counts") == preflight.get("matched_counts")
    assert manifest.get("inputs", {}).get("matched_files") == 7
    input_refs = manifest.get("inputs", {}).get("files") or []
    assert {item.get("file_name") for item in input_refs} >= {
        "LSV_good.txt",
        "LSV_bad.txt",
        "CV_demo.txt",
        "EIS_demo.txt",
    ...
    assert manifest.get("parameters", {}).get("lsv_target_current") == "10,100"
    assert manifest.get("results", {}).get("normalized_results", 0) >= 1
    ...
    assert any(path.name.endswith("_CV.png") for path in output_files)
    assert any(path.name.endswith("_EIS_Nyquist.png") for path in output_files)
    assert any(path.name.endswith("_EIS_Bode.png") for path in output_files)
    assert any(path.name.endswith("_LSV.png") for path in output_files)
    assert any(path.name.endswith("_ECSA.png") for path in output_files)
